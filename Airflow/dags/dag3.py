...
def process_recent_csv_from_s3(bucket_name, **kwargs):
        ...
        df = df.dropna(subset=['Extracted Text', 'Response'])  # Drop rows with NaN
        df = df[
            df['Extracted Text'].str.strip() != ''  # Drop rows with empty 'Extracted Text'
        ]
        df = df[
            df['Response'].str.strip() != ''  # Drop rows with empty 'Response'
        ]
...
def cc_pinecone(dataframe):
    ...
    for idx, row in dataframe.iterrows():
        ...
        question_text = row.get('Extracted Text', "").strip()
        answer_text = row.get('Response', "").strip()
